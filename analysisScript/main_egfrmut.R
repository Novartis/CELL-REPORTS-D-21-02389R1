#' ---
#' title:  QC analysis of screen of EGFR mutant cell lines
#' author: Grainne Kerr
#' output:
#'    html_document:
#'        toc: true
...
# rmarkdown::render('main_egfrmut.R', output_format = 'all', clean = TRUE)
...
CELL_LINES <- c("HCC-2279", "HCC2935", "HCC4006", "HCC827", "NCI-H1650", "NCI-H3255", "PC-14", "PC-3")
REMOVE_DOSAGE <- list(CELLLINE = c("HCC-2279", "NCI-H3255"), DOSAGE = 100)

INVESTIGATION_NUM <- 160
...
#' This resulted in 378 pan lethals and 1221 negative controls.
#'
...
X <- read.delim("http://nrchbs-slp0083.nibr.novartis.net:8080/GDSx/rest/api/spotfire/cEPIC/getInvestigations")

X <- X %>% filter(INVESTIGATION_ID == INVESTIGATION_NUM) %>% unique()
...
samp.list2 <- X %>% select(PHYSICAL_SMF_ID) %>% unique()
ctrl.samp.list <-
  X %>% select(CONTROL_SAMPLE_ID) %>% pull(CONTROL_SAMPLE_ID)
ctrl.samp.list2 <-
  X %>% select(CONTROL_SAMPLE_PHYSICAL_SMF_ID) %>% unique()
...
logFC = lapply(selectedSamplesAndControls, function(x) {
  out <- tryCatch({
    url = paste0(
      "http://nrchbs-slp0083.nibr.novartis.net:8080/GDSx/rest/api/spotfire/cEPIC/getInvestigationLogFC/",
      x
    )
    read.delim2(url)
  },
...
logFC %<>% dplyr::mutate(BIOSPECIMEN = str_replace(BIOSPECIMEN, "NCI-1650", "NCI-H1650"))
logFC %<>% dplyr::filter(!(DOSAGE==REMOVE_DOSAGE$DOSAGE & BIOSPECIMEN %in% REMOVE_DOSAGE$CELLLINE))
logFC %<>% dplyr::mutate(CLEANNAME = str_replace(tolower(BIOSPECIMEN), "-", ""))
logFC %<>% filter(BIOSPECIMEN %in% CELL_LINES)
...
rsaLogFC = lapply(selectedSamplesAndControls, function(x) {
  out <- tryCatch({
    url = paste0(
      "http://nrchbs-slp0083.nibr.novartis.net:8080/GDSx/rest/api/spotfire/cEPIC/getInvestigationRSALogFC/",
      x
    )
    read.delim2(url)
  },
...
#'
#' The figure below illustrates the "window" for identifying hits observed by looking at the control gene information of pan lethal (green) vs negative control genes(red/brown). Note that TMM normalization was used in the analysis pipeline. Note that HCC2935 has the smallest window and was only screened with one pool.
#' 
#'
...
plotDataQC_panLethalControls[["qcdataret"]] %>%
  ...
  dplyr::mutate(SampleName = str_replace(SampleName, "(.*)-Cas9.*", "\\1")) %>%
  dplyr::mutate(SampleName = str_replace(SampleName, "(.*)_EF1aL.*", "\\1")) %>%
  dplyr::mutate(SampleName = str_replace(SampleName, "(.*) EF1a.*", "\\1")) %>%
  dplyr::mutate(SampleName = str_replace(SampleName, "(.*)-CMV-.*", "\\1")) %>%
  dplyr::mutate(SampleName = str_replace(SampleName, "(.*)-cas9.*", "\\1")) %>%
...
#' This cumulative plot shows that 21/22 of the control arms have a SSMD score of -1 or better. In the treatment arm 16/22 have a score of -1 or better.
#' 
#+ fig.height = 6, fig.width = 10, label = 'cumulativePlotZPrimeScorePanLethals'

ggplot(setDT(plotDataQC_panLethalControls[["dat"]])[,y2:=cumsum(y),"PANEL"],aes(x=x)) +
...
#' #### Correlation between treated and control arms
#' 
#' We examined the knockdown of the essential genes and observe a high correlation between pan-lethal genes in both the treated and control arms of the experiment (the effect of lethal genes should be similar in both arms), the lowest correlation observed in the HCC3255 and HCC827 cell lines.
#' 
#' 

#+ fig.height = 12, fig.width = 10, label = 'panLethalGeneCorrelation'
logFC  %>%
...
p1 <- ggcorr(data=NULL, 
             cor_matrix = cormat_p1, geom = "blank", label = TRUE, label_round = 2, size=3, hjust=0.2, vjust=1, angle=90) +  
...
p2 <- ggcorr(data=NULL, 
             cor_matrix = cormat_p2, geom = "blank", label = TRUE, label_round = 2, size=3, hjust=0.2, vjust=1, angle=90) +  
  geom_point(aes(size = coefficient, color = coefficient > 0, alpha = ifelse(abs(coefficient) > 0.8, "HIGH", ifelse(abs(coefficient) > 0.5, "MED","LOW")))) +
  scale_size_continuous(range = c(5, 12)) + 
  scale_alpha_manual(values = c("HIGH" = 0.5, "MED"=0.25, "LOW" = 0)) +  
  guides(color = "none", alpha = "none")+ 
...
p3 <- ggcorr(data=NULL, 
             cor_matrix = cormat_p3, geom = "blank", label = TRUE, label_round = 2, size=3, hjust=0.2, vjust=1, angle=90) +  
  geom_point(aes(size = coefficient, color = coefficient > 0, alpha = ifelse(abs(coefficient) > 0.8, "HIGH", ifelse(abs(coefficient) > 0.5, "MED","LOW")))) +
  scale_size_continuous(range = c(5, 12)) + 
  scale_alpha_manual(values = c("HIGH" = 0.5, "MED"=0.25, "LOW" = 0)) +  
  guides(color = "none", alpha = "none")+ 
...
# # Create a list for the input to upsetR
 listInput_HCC2279 <- logFC  %>%
   filter(BIOSPECIMEN == "HCC-2279") %>%
   group_by(Gene, BIOSPECIMEN) %>% 
   dplyr::summarize(logFC = mean(XTreatment_Control_LogFC, na.rm=T)) %>%
   filter(logFC < -0.5) %>%
   pull(Gene)
 
 listInput_HCC2935 <- logFC  %>%
   filter(BIOSPECIMEN == "HCC2935") %>%
   group_by(Gene, BIOSPECIMEN) %>% 
   dplyr::summarize(logFC = median(XTreatment_Control_LogFC, na.rm=T)) %>%
   filter(logFC < -0.5) %>%
   pull(Gene)
 
 listInput_HCC4006 <- logFC  %>%
   filter(BIOSPECIMEN == "HCC4006") %>%
   group_by(Gene, BIOSPECIMEN) %>% 
   dplyr::summarize(logFC = median(XTreatment_Control_LogFC, na.rm=T)) %>%
   filter(logFC < -0.5) %>%
   pull(Gene)
 
 listInput_HCC827 <- logFC  %>%
   filter(BIOSPECIMEN == "HCC827") %>%
   group_by(Gene, BIOSPECIMEN) %>% 
   dplyr::summarize(logFC = median(XTreatment_Control_LogFC, na.rm=T)) %>%
   filter(logFC < -0.5) %>%
   pull(Gene)
 ...
 listInput_NCIH1650 <- logFC  %>%
   filter(BIOSPECIMEN == "NCI-H1650") %>%
   group_by(Gene, BIOSPECIMEN) %>% 
   dplyr::summarize(logFC = median(XTreatment_Control_LogFC, na.rm=T)) %>%
   filter(logFC < -0.5) %>%
   pull(Gene)
 
 listInput_NCIH3255 <- logFC  %>%
   filter(BIOSPECIMEN == "NCI-H3255") %>%
   group_by(Gene, BIOSPECIMEN) %>% 
   dplyr::summarize(logFC = median(XTreatment_Control_LogFC, na.rm=T)) %>%
   filter(logFC < -0.5) %>%
   pull(Gene)
 
 listInput_PC14 <- logFC  %>%
   filter(BIOSPECIMEN == "PC-14") %>%
   group_by(Gene, BIOSPECIMEN) %>% 
   dplyr::summarize(logFC = median(XTreatment_Control_LogFC, na.rm=T)) %>%
   filter(logFC < -0.5) %>%
   pull(Gene)
 ...
 listInput <- list(HCC2279 = listInput_HCC2279, 
                   HCC2935 = listInput_HCC2935, 
                   HCC4006 = listInput_HCC4006, 
                   HCC827 = listInput_HCC827, 
                   NCIH1650 = listInput_NCIH1650, 
                   NCIH3255 = listInput_NCIH3255, 
                   PC14 = listInput_PC14,
                   PC3 = listInput_PC3)

#' ### Synergistic Genes
#'
#' The gene score distribution plots below show the distribution of the logFC essential/panlethal genes of the treated/Untreated. This corresponds the the treatment effect, and we would expect both to be ~0 (no difference in effect of pan lethals in the treated and untreated arm.) The distributions all center around 0, with the exception of the HCC827 cell line. Here the pan -lethals are skewed to the right, indicating that the lethal genes are more effective in the untreated arm compared to the treated arm ("pseudo-rescuer")
#'
 
#+ fig.height = 6, fig.width = 14, label = 'logFCDistribution'
 logFC %>%
   mutate(CONTROL_GENE = ifelse(
     Gene %in% panLethals[[1]],
     "Pan-lethal",
...
#'
#' The proportion of genes which have a logFC < -0.5 and which genes are common across the cell lines. The upset plot shows the number synergistic genes in each cell line (set) (logFC <= - 0.5), and the number in each intersection thereof.
#' 
#' 
...
#' 
#' The genes which were synergistic in >=2 cell lines.
#' 
